<?php
/*
 * Plugin Name: Featured Posts Sidebar Widget
...
class TI_Featured_Posts_Sidebar extends WP_Widget {
			...
			<?php while ( $ti_featured_posts->have_posts() ) : $ti_featured_posts->the_post(); ?>
				<li>
					<figure class="entry-image">
						<a href="<?php the_permalink(); ?>?utm_source=sidebar_featured_posts&utm_medium=image_link&utm_campaign=zkk_internal">
						<?php 
						if ( has_post_thumbnail() ) {
							the_post_thumbnail( 'rectangle-size' );
						} elseif( first_post_image() ) { // Set the first image from the editor
							echo '<img src="' . first_post_image() . '" class="attachment-masonry-size wp-post-image" />';
						}?>
					</a>
					</figure>
