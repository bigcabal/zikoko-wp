...
h1, h2, h3, h4, h5, h6, .tagline, .sub-title, .menu a, .widget_pages, .widget_categories, .entry-meta, .entry-note, .read-more, #submit, .single .entry-content > p:first-of-type:first-letter, input#s, .widget_ti-about-site p, .comments .vcard, #respond label, .copyright, #wp-calendar tbody, .score-box .total {font-family: '<?php echo $ti_option['font_titles']; ?>', Arial, Verdana, 'Helvetica Neue', Helvetica, sans-serif; font-weight:400;}
...
.posts-slider figure:before {background-color:#18191a;opacity:<?php echo $ti_option['slider_tint_strength']; ?>;}
<?php } elseif ( $ti_option['slider-tint'] == 'slider-tint-main' ) { ?>
.posts-slider figure:before {background-color:<?php echo $main_site_color; ?>;opacity:<?php echo $ti_option['slider_tint_strength']; ?>;}
<?php } ?>
